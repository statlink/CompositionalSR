alfa.esf <- function(y, x, a, coords, xnew = NULL, coordsnew, yb = NULL) {
  ...
  if ( a <= 1e-5 ) {
    ...
    if ( length(sela) > 0 )  sela <- .areggompesf(y, x[, -1], X, a, ya, kld)[-1, 1]
  ...
  list(runtime = runtime, be = be, gama = gama, ESF = sela, X.esf = X[, sela, drop = FALSE], dev = mod$deviance, est = est)
}
